...
# ============================================================
# تشخيص إنتاجي 2026-09-05 (بلاغ المُشغّل «الفلتر زبالة»): ثلاثة أنماط
# قمامة كانت تجتاز شلال القبول:
#   (أ) إعادة توجيه لمزوّد — «الي يبغى رقم مختص… يرسل لي خاص»:
#       المرسل يملك المنفّذ ويُعلن عنه (إعلان لا طلب) — الرغبة محرّفة
#       لطرف ثالث (اللي يبغى/لمن يود) + إحالة تواصل للمرسل نفسه.
#   (ب) تعليمات حدث — «لمن يود المشاركة على المايك… 1) يكتب رقم
#       𝟭𝟬𝟬»: فعل تنفيذ + رقم = قاعدة تفويض معزولة بلا مفعول أكاديمي.
#   (ج) سرد شخصي — «مافيه شي يرقع اللي يسويه»: فعل ملتبس (يسويه)
#       + مبتدأ نسبي بلا استفهام ولا رغبة.
# الحل الجذري (ليس ترقيعًا): قبول التفويض يتطلب إشارة طالب من
# الطرف الأول — إما رغبة صريحة (ابي/ابغا/محتاج…) أو استفهام حقيقي
# (مين/احد/من عنده/؟). رغبة الطرف الثالث المعرّفة (اللي يود…) +
# إحالة تواصل = عرض خدمة مقلوب. لا رغبة ولا استفهام = سرد لا طلب.

# رغبة المتكلم (الطالب نفسه) — كلمات مستقلة
FIRST_PERSON_WANT = (
    'ابي', 'ابغي', 'ابغا', 'ابغى', 'اب', 'اريد', 'ودي',
    'بغيت', 'بغي', 'محتاج', 'احتاج', 'حتاج', 'ابغيها', 'وديها',
)
# استفهام الطرف الأول (who-words + صيغ من-الاستفهامية + ؟)
FIRST_PERSON_QUESTION_WORDS = (
    'مين', 'منو', 'احد', 'حدا', 'احدي', 'شخص', 'واحد', 'حد',
)
FIRST_PERSON_QUESTION_PATTERNS = (
    'من عنده', 'من يعرف', 'من يقدر', 'من يسوي', 'من يحل',
    'من يرسو', 'من عندي', 'تعرفون', 'تعرف احد', 'مين يعرف',
)
# رغبة محرّفة لطرف ثالث (صيغة المُعلن: «الي يبغى…/لمن يود…»)
EMBEDDED_THIRD_PARTY_WANT = (
    'اللي يبغى', 'الي يبغى', 'اللي يود', 'الي يود', 'لمن يود',
    'اللي يريد', 'الي يريد', 'اللي يحتاج', 'الي يحتاج',
    'من يبغى', 'من يود', 'من يريد', 'من يحتاج',
)
# إحالة تواصل للمرسل (مؤشر الإعلان/الوسيط)
CONTACT_REDIRECT_MARKERS = (
    'يرسل لي', 'يرسلني', 'راسلني', 'تواصل معي', 'كلمني',
    'ارسل لي', 'راسله', 'يراسلني', 'كلمني خاص', 'يرسل لي خاص',
)

# [ADMIN-NOTICE-v4.4.3] بلاغ الإنتاج 2026-09-05 (02:59): تحذير إداري اجتاز
# الشلال كطلب — «القروبات ف البلاك بورد دوروها ممنوعه هنا .. ف كرماً لا
# احد يطلب قروب هنا او يكتب رقم الشعب». الجذر: المحرك لا يفهم النفي —
# «لا احد» تُحسب إشارة استفهام (احد!) و«يكتب رقم» تفويض → ACCEPT 0.93.
# الإصلاح الجذري: النفي/المنع إشارة قاعدة إدارية لا طلب — طالما لا
# رغبة صريحة من المُرسل نفسه (ابي/ابغى/محتاج) ولا استفهام حقيقي (؟).
PROHIBITION_MARKERS = (
    'ممنوع',                       # ممنوع/ممنوعه/الممنوع/ممنوعات
    'يمنع', 'محظور', 'لا يسمح', 'غير مسموح',
    'كرما لا', 'كريما لا',          # كرماً لا (التشكيل يُزال بالتوحيد)
    'الرجاء لا', 'الرجاء عدم', 'برجاء عدم',
    'تذكير لا', 'تذكير بعدم', 'تنبيه لا', 'تنبيه بعدم',
    'لا تنشر', 'لا ترسل', 'لا تكتب', 'لا تحط',
)
# نفي موجّه للمجموعة: «لا احد يطلب/يكتب/يرسل..»، «ما احد ي..»،
...
class RuleBasedIntentClassifier:
    ...
    def classify_sync(self, text: str) -> IntentDecision:
        ...
        # ---- 0) [GENIUS-PLAN-v4.4.2] بوابات القمامة قبل الشلال ----
        _fwant = _has_any(t, FIRST_PERSON_WANT)
        # ---- 0-ج) [ADMIN-NOTICE-v4.4.3] تحذير إداري/قاعدة قروب ----
        # «ممنوع هنا.. لا احد يطلب قروب» ليست طلبًا: النفي/المنع يشير
        # لقاعدة المجموعة. يُستثنى: رغبة صريحة من المُرسل (ابي/ابغى..)
        # أو استفهام حقيقي (؟) — «مافيه احد يقدر يحل التفاضل؟» يبقى طلبًا.
        _proh = _has_sub(t, PROHIBITION_MARKERS)
        _negdir = NEG_DIRECTIVE_RE.search(t) is not None
        if (_proh or _negdir) and not _fwant \
        ...
        if _contact and (_relwant or not _fwant):
            return _dec("REJECT", 0.96, "service_offer",
                        "إحالة تواصل لمزوّد (رغبة طرف ثالث/لا طلب شخصي) — إعلان")
        # (ب) لا طالب من الطرف الأول إطلاقًا: لا رغبة ولا استفهام —
        # سرد شخصي أو تعليمات حدث، حتى لو وُجد فعل تنفيذ ملتبس
        if not _fwant and not _q:
            return _dec("REJECT", 0.90, "non_request_statement",
                        "سرد/تعليمات بلا رغبة أو استفهام من المُرسل — ليس طلبًا")
