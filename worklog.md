...
Task: متابعة تحسين Request Filter بعد الجلسة السابقة (كشف سبام أقوى: أرقام جوال/@handles/صياغة إعلانية بدل الاعتماد على كلمة "تقرير" فقط) ثم commit + push.

Work Log:
- استعادة المستودع: الساندبوكس أُعيد ضبطه بين الجلسات — /home/z/wlm-fix و/tmp/wlm كانا مفقودين. أُعيد استنساخ المستودع من github.com/azammaza770309310-boop/whatsapp-link-monitor إلى /tmp/wlm ثم نُسخ إلى /home/z/wlm (التوكن استُخدم inline في clone ثم نُظّف من .git/config فورًا — لم يُخزَّن في أي ملف).
- تشخيص نقاط الضعف الحقيقية بفحص حالات حافة (edge cases):
  1. [CRITICAL] _PHONE_RE القديم `\b05\d{8}\b` يفوّت الأرقام الملتصقة بأحرف عربية لأن `\b` لا يعمل بين حرف عربي (\w) ورقم: «رقمي0540916687» و«للتواصل0540916687» → phone=False. ثغرة يستغلها المُعلنون للالتفاف.
  2. [هشاشة] تركيبة هاتف+@handle كانت تعطي provider=6 بالضبط على عتبة الرفض (3+2+1) — هشة؛ «0540916687 @DrMed» ترفض بفارق صفر.
  3. إشارات مُعلنين شائعة مفقودة: «ننجز/نساعدكم/مكتبنا/فريقنا/للاستفسار» و«تنافسية/مجاني/تخفيضات/حسم/5 نجوم/هدية».
- التحسينات المنفّذة (request_filter.py):
  * FILTER_VERSION: "v2" → "v2.1".
  * _PHONE_RE: استبدال `\b05\d{8}\b` بـ `(?<!\d)05\d{8}(?!\d)` — التقاط الأرقام الملتصقة بأحرف عربية مع منع مطابقة جزء من رقم أطول.
  * PROVIDER_INDICATORS += 9 عبارات: ننجز، ننجز لك، نساعدكم، نساعدكم في، مكتبنا، فريقنا، للاستفسار، للاستفسارات، للحجز والاستفسار.
  * ADVERTISEMENT_STRONG_SIGNALS += 13 عبارة: تخفيضات، حسم، تنافسية، بأسعار تنافسية، تنافسي، مجاني، مجانا، هديه، هدية، هدية مجانية، الباقة، باقات، اشتراك، تقييم 5، تقييم خمس نجوم، 5 نجوم، موصى به.
  * تركيبة phone+@handle: تعزيز من +2 إلى +4 (الأرضية الآن 8 فوق العتبة 6 بمسافة آمنة). أُثبت أن SEEKER_WITH_CONTACT الحقيقية لا تحوي @handle (فقط رقم) فلا تتأثر.
- اختبارات جديدة في verify_request_filter.py (+9 cases، الآن 52/52):
  * قسم REJECT_SPAM_EDGE: «للتواصل0540916687»، «رقمي للتواصل 0540916687 @DrMedical»، «0540916687 @DrMed»، «+966540916687 @DrMed»، إعلان التقرير الطبي المزوّر الأصلي، إشارات v2.1 الجديدة، dotted-obfuscation + هاتف ملتصق.
- التحقق الكامل: 52/52 cases pass (43 الأصلية + 9 الجديدة). imports clean. FILTER_VERSION=v2.1.
- فحص عدم الكسر: bot.py يستورد analyze_request/RequestAnalysis/FILTER_VERSION/FILTER_MODE — كلها محفوظة (لم يُغيّر أي توقيع). AST/imports OK.

Stage Summary:
- النتيجة: نقطة ضعف حرجة أُصلحت (هاتف ملتصق بعربية)، هشاشة عتبة الرفض زُالت (6→8 أرضية)، 22 إشارة إعلانية جديدة، 9 اختبارات سبام متقدّم.
- مبدأ التصميم المُتّبع: الكشف القوي بالتركيز على الإشارات المركّبة (phone+@handle) والإشارات التجارية الواضحة (صيغة الجمع ننجز/نساعدكم + مكتبنا/فريقنا)، وليس بالاعتماد على كلمة أكاديمية منفردة مثل «تقرير» — وهذا يحل بالضبط نقطة الضعف المُشار إليها في الجلسة السابقة.
