...
"""
Telegram WhatsApp Link Monitor - v10
البوت البسيط - يراقب المجموعات التي يُضاف إليها كـ admin

المميزات:
1. ✅ سحب روابط واتساب فقط (جميع الأنواع)
2. ✅ واجهة أزرار تفاعلية (Inline Keyboard)
3. ✅ استبعاد روابط خدمات طلابية معروفة
4. ✅ دعوة الأصدقاء عبر الرابط
5. ✅ إضافة البوت للمجموعات كـ Admin
6. ✅ مسح تاريخي تلقائي عند الإضافة
"""
...
DATA_DIR = "data"
LOGS_DIR = "logs"
DB_FILE = os.path.join(DATA_DIR, "whatsapp_links.db")
...
# روابط خدمات طلابية معروفة (تُستبعد تلقائياً)
# هذه خدمات مدفوعة تبيع حلول للمواد (سبام)
STUDENT_SERVICES_BLACKLIST = [
    ...
    'answers', 'answer',
    'kau_services', 'kau_services_',
    'imamu_services', 'imam_services',
    'seu_services', 'seu_help',
    'taif_services', 'taif_uni',
    'majmah_services',
    'stu_services', 'stu_help',
    # خدمات مدفوعة معروفة
    'talib', 'taleb', 'talebin',
    'dr_talib', 'drtalib',
    'eshterak', 'eshtirak',
    ...
    'library_paid',
    # قنوات ترويجية مكررة
    'ads_channel', 'ads_only',
    'free_ads',
]

# keywords للكشف عن خدمات مدفوعة في الرابط
...
class DatabaseManager:
    ...
    async def count_friends(self):
        ...
        row = await cur.fetchone() if (cur := await conn.execute("SELECT COUNT(*) FROM friends")) else None
        return row[0] if row else 0
...
# -------------------------------------------------------------------
# WhatsApp Link Extractor + Filter
# -------------------------------------------------------------------


class LinkExtractor:
    """يستخرج روابط واتساب ويفلتر خدمات الطلاب"""
    ...
    def is_student_service(link: str) -> bool:
        """يتحقق إن كان الرابط لخدمة طلابية مدفوعة (تُستبعد)"""
    ...
    @staticmethod
    def filter_links(links: List[str]) -> Tuple[List[str], List[str]]:
        """يقسم الروابط إلى (صالحة، مستبعدة)"""
        valid = []
        excluded = []
        for link in links:
...
class MessageFormatter:
    ...
    def format_link_message(group_name, sender_name, message_date, links, message_text, message_link=None):
        ...
        if message_link:
            lines.extend(["", f"📨 الرسالة: {message_link}"])
        lines.extend(["", "💬 الرسالة الأصلية:", message_text])
        return "\n".join(lines)
    ...
    def format_welcome(bot_username):
        return (
            "🤖 أهلاً بك في بوت سحب روابط واتساب!\n\n"
            "📚 ماذا يفعل هذا البوت؟\n"
            "• يراقب مجموعاتك الدراسية\n"
            "• يستخرج روابط واتساب منها\n"
            "• يستبعد خدمات الطلاب المدفوعة\n"
            "• ينشر الروابط في قناة مشتركة\n\n"
            "🚀 اختر من القائمة:"
        )

    @staticmethod
    def format_status(total_links, groups_count, users_count, friends_count, scan_running, scan_progress=""):
        return (f"📊 حالة البوت\n\n"
    ...
    def format_help():
        return (
            "🤖 دليل الاستخدام\n\n"
            "📌 كيف يعمل البوت؟\n\n"
            "1️⃣ اضغط زر «إضافة البوت لمجموعتك»\n"
...
class Monitor:
    ...
    def _register_handlers(self):
        ...
        # 1. رسائل جديدة في كل المجموعات
        self.bot_client.add_event_handler(
            self._on_new_message,
            events.NewMessage()
        )

        # 2. البوت أُضيف/أُزيل من مجموعة
        self.bot_client.add_event_handler(
            self._on_chat_action,
            events.ChatAction()
        )

        # 3. أوامر الدردشة الخاصة
        self.bot_client.add_event_handler(
            self._on_private_message,
            events.NewMessage(func=lambda e: e.is_private)
        )

        # 4. ضغطات الأزرار (Callback Queries)
        self.bot_client.add_event_handler(
            self._on_callback,
            events.CallbackQuery()
        )

        # 5. أوامر القناة (للمالك)
        self.bot_client.add_event_handler(
        ...
        logging.info("Bot handlers registered (groups + private + buttons + channel)")

    async def _get_main_menu(self):
        """القائمة الرئيسية - أزرار"""
        return [
            [Button.inline("➕ إضافة البوت لمجموعتك", b"add_to_group")],
            [Button.inline("👥 دعوة صديق", b"invite_friend"),
             Button.inline("👨‍💼 أصدقائي", b"my_friends")],
            [Button.inline("📊 حالة البوت", b"status"),
             Button.inline("❓ المساعدة", b"help")],
            [Button.url("📢 القناة", f"https://t.me/c/{str(self.config.channel_id).replace('-100', '')}")]
        ]
    ...
    async def _on_new_message(self, event):
        try:
            ...
            valid_links, excluded_links = LinkExtractor.filter_links(links)
            if excluded_links:
                logging.info(f"[LIVE] Excluded {len(excluded_links)} student service links in {group_name}")
    ...
    async def _on_private_message(self, event):
        ...
        try:
            ...
            # لو المستخدم في حالة إضافة صديق
            ...
            if text.startswith("/start") or text == "/start":
                me = await self.bot_client.get_me()
                await event.reply(
                    MessageFormatter.format_welcome(me.username),
            ...
            if text == "/status":
                ...
                await event.reply(MessageFormatter.format_status(
                    total, groups, users, friends, self.is_scan_running(), self._scan_progress))
                return

            # رسالة افتراضية - عرض القائمة
            if text and not text.startswith('/'):
                me = await self.bot_client.get_me()
                await event.reply(
                    "👋 أهلاً! اختر من القائمة:",
                    buttons=await self._get_main_menu()
                )
    ...
    async def _on_callback(self, event):
        ...
        try:
            ...
            if data == "add_to_group":
                ...
                share_link = f"https://t.me/{bot_username}?start=ref_{sender_id}"
                ...
                await event.edit(
                    ...
                    "2️⃣ أضف رقم صديقك يدوياً (لإرسال دعوة تلقائية):\n"
                ...
                await event.edit(
                    ...
                    "مثال: +966500000000\n\n"
                    "⚠️ سيُرسل له رابط البوت تلقائياً.\n\n"
                    "للإلغاء: /cancel",
                ...
                await event.edit(
                    MessageFormatter.format_welcome((await self.bot_client.get_me()).username),
                    buttons=await self._get_main_menu()
                )
    ...
    async def _handle_add_friend_input(self, event, sender, text):
        ...
        # محاولة إرسال دعوة عبر تيليجرام (لو الرقم مرتبط بحساب تيليجرام)
        try:
            # إرسال رسالة للصديق (لو ممكن)
            me = await self.bot_client.get_me()
            bot_username = me.username or "YourBot"
            invite_text = (
            ...
            # البوتات لا يمكنها إرسال رسائل لغير من بدأهم. سنحاول عبر phone.
            # لكن هذه الميزة محدودة في تيليجرام
            try:
                # محاولة إرسال للرقم (قد تفشل لو لم يبدأ المستخدم البوت)
                ...
                # لو فشل الإرسال، نعطي المستخدم الرابط لمشاركته يدوياً
                await event.reply(
                    f"⚠️ تم حفظ {phone} في قائمة الأصدقاء.\n\n"
                    f"❌ لم أستطع إرسال دعوة تلقائية (قد يكون لديه إعدادات خصوصية).\n\n"
...
async def main():
    ...
    logging.info("=== Telegram WhatsApp Link Monitor v10 ===")
