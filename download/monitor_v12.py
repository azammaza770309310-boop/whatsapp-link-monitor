...
class Monitor:
        ...
        try:
            ...
            for link_info in links:
                ...
                if is_new:
                    ...
                    logging.info(f"[QUEUE] NEW link from {group_name}: {link_info['raw'][:50]}")
                else:
                    await self.metrics.record_duplicate()
                    logging.debug(f"[QUEUE] Duplicate skipped: {link_info['normalized']}")
            ...
            if cmd == "/help": await reply(MessageFormatter.format_help())
                ...
                if len(parts) < 2:
                    ...
                    async with self.db._lock:
                        conn = await self.db._ensure_conn()
                        await conn.execute("UPDATE watchers SET joiner_enabled = 1 WHERE phone = ?", (target_phone,))
                        await conn.commit()
                ...
                if len(parts) < 2:
                    ...
                    async with self.db._lock:
                        conn = await self.db._ensure_conn()
                        await conn.execute("UPDATE watchers SET joiner_enabled = 0 WHERE phone = ?", (target_phone,))
                        await conn.commit()
                    await reply(f"⏸️ تم إيقاف الانضمام للحساب: {target_phone}")

            elif cmd == "/join_status":
                # === تقرير حالة كل حساب فدائي ===
                joiners = await self.db.get_watchers_by_role("joiner")
                if not joiners:
                    ...
                    for j in joiners:
                        ...
                        # joiner_enabled
                        conn = await self.db._ensure_conn()
                        cursor = await conn.execute("SELECT joiner_enabled, last_join_timestamp, health_score FROM watchers WHERE phone = ?", (jphone,))
                        row = await cursor.fetchone()
                        enabled = bool(row[0]) if row and row[0] is not None else True
                        last_join_ts = row[1] if row else None
                        health = row[2] if row else 100
        ...
        while self._running:
            try:
                ...
                if state in (GroupState.JOINED, GroupState.ALREADY_MEMBER):
                    logging.info(f"[SCHED] {normalized} already {state} — skipping")
                ...
                if state == GroupState.BANNED:
                    logging.info(f"[SCHED] {normalized} is BANNED — skipping")
                    await self.prod_db.update_queue_status(link_data['id'], 'DONE')
                ...
                if state == GroupState.DISCOVERED or state is None:
                    ...
                    # === نشر الرابط في القناة بعد موافقة AI ===
                    inserted = await self.db.insert_request(
                    ...
                    if inserted:
                        ...
                        logging.info(f"[SCHED] 📢 Published to channel: {raw_link[:50]}")
                ...
                if not guard_ok:
                    logging.info(f"[GUARD] Blocked {phone} from joining {raw_link[:50]}: {guard_reason}")
                    ...
                    if 'floodwait' in guard_reason:
                        ...
                        if floodwait_until:
                            ...
                            logging.info(f"[GUARD] FloodWait retry after {next_retry_dt.strftime('%H:%M:%S')}")
                        else:
                            await self.prod_db.update_queue_status(link_data['id'], 'QUEUED',
    ...
    async def _safety_guard(self, phone: str, normalized_link: str, link_data: dict) -> Tuple[bool, str]:
        ...
        cursor = await conn.execute(
            "SELECT last_join_timestamp FROM watchers WHERE phone = ?", (phone,))
        row = await cursor.fetchone()
        if row and row[0]:
            try:
                last_join = datetime.fromisoformat(row[0]) if isinstance(row[0], str) else row[0]
                elapsed = (datetime.now() - last_join).total_seconds()
    ...
    async def _get_daily_limit(self, phone: str) -> int:
        ...
        joiner: 10, backup: 5, monitor: 0
        """
        # اقرأ من env var بصيغة DAILY_LIMIT_<PHONE_LAST_4>=N
        # أو استخدم الافتراضي حسب role
        conn = await self.db._ensure_conn()
        cursor = await conn.execute("SELECT role FROM watchers WHERE phone = ?", (phone,))
        row = await cursor.fetchone()
        role = row[0] if row else 'monitor'
    ...
    async def _join_group_safe(self, client, link_data: dict, phone: str):
        ...
        cursor = await conn.execute("SELECT joiner_enabled FROM watchers WHERE phone = ?", (phone,))
        row = await cursor.fetchone()
        if row and row[0] == 0:
...
async def main():
    ...
    try:
        ...
        logging.info(f"   Total accounts in Supabase: {len(all_accounts)}")
        logging.info(f"   Monitors: {len(monitors)}")
        logging.info(f"   Joiners: {len(joiners)}")
        for w in all_accounts:
            logging.info(f"   → {w['phone']} (role={w.get('role', 'monitor')})")
    except RuntimeError as e:
        logging.critical(f"❌ FATAL: {e}")
        logging.critical("❌ Bot will NOT start. Add accounts via /login first.")
        sys.exit(1)
