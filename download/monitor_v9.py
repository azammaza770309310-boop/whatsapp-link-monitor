...
class HelpRequestDetector:
    """
    كاشف ذكي لطلبات المساعدة بدون كلمات مفتاحية صريحة.
    يعتمد على:
    1. علامات الاستفهام (؟ / ?)
    2. صيغ الطلب (ممكن، ابي، اريد، أبغى، احتاج)
    3. السياق الدراسي (إشارات لمهام، اختبارات، مواد)
    4. طول الرسالة (ليست قصيرة جداً ولا طويلة جداً)
    5. ليست سبام/إعلان
    """

    # علامات الاستفهام العربية والإنجليزية
    QUESTION_MARKS = ['؟', '?', '??', '???']

    # صيغ الطلب (للتحقق من النية)
    REQUEST_PATTERNS = [
        r'\b(ممكن|ممكنن|ممكنة)\b',
        r'\b(ابي|أبي|ابغى|ابغ|أبغى|اري|أريد|اريد|احب|أحب)\b',
        r'\b(احتاج|أحتاج|محتاج)\b',
        r'\b(طلبتكم|طلبكم|طلب|طلبات)\b',
        r'\b(مساعدة|ساعدوني|ساعدني|ساعدو|اعانوني|أعانوني)\b',
        r'\b(مين|من|مَن)\s+(يعرف|يقدر|يساعد|عندة|عنده)\b',
        r'\b(هل|هل في|هل عندكم|هل أحد|هل احد)\b',
        r'\b(كيف|وش|ويش|ايش|إيش|شنو|شلون)\b',
        r'\b(ابحث|أبحث|محتاج مساعدة|محتاجين)\b',
        r'\b(عارف|يعرف|تعرفون|تعرف|تقدرون|تقدر)\b',
        r'\b(help|please|need)\b',
    ]

    # سياق دراسي (إشارات للمواد والمهام)
    ACADEMIC_CONTEXT = [
        # مهام
        'واجب', 'واجبات', 'اسايمنت', 'اسمنت', 'assignment', 'homework', 'hw',
        'بحث', 'ابحاث', 'research', 'report', 'تقرير',
        'عرض', 'برزنتيشن', 'بريزنتيشن', 'presentation',
        'مشروع', 'project', 'مشروع تخرج', 'تخرج',
        'سكليف', 'اسكليف', 'سكيرف',
        # اختبارات
        'فاينل', 'final', 'نهائي', 'كويز', 'quiz', 'اختبار', 'اختبارات', 'ميدتيرم', 'midterm',
        'استماع', 'محاضرة', 'محاضرات', 'ملخص', 'خلاصة', 'ملخصات',
        # مواد دراسية
        'مادة', 'مواد', 'كورس', 'course', 'تخصص', 'قسم', 'كلية', 'جامعة',
        'دكتور', 'دكتورة', 'استاذ', 'أستاذ', 'professor', 'prof',
        'فصل', 'ترم', 'سمستر', 'semester', 'تسجيل',
        # مهارات
        'تفسير', 'شرح', 'توضيح', 'حل', 'حلول', 'اجابة', 'إجابة',
        # أنواع المساعدة
        'بمقابل', 'مقابل', 'بأجر', 'باجر', 'مدفوع', 'مجان', 'مجاناً', 'مجاني',
        'مختص', 'خبير', 'محترف', 'مدرس', 'معلم',
    ]

    # سبام للتجاهل
    SPAM_INDICATORS = [
        'buy now', 'limited offer', 'click here', 'اضغط هنا',
        'follow me', 'تابعني', 'اشتراكي', 'يوتيوب', 'tiktok',
        'https://t.me/+', 'joinchat', 'ربح', 'كاش', 'money',
        'كورس مجاني', 'تطبيق مجاني', 'تخفيض', 'عرض خاص',
    ]

    @classmethod
    def is_help_request(cls, text: str, min_length: int = 15, max_length: int = 2000) -> Tuple[bool, List[str]]:
        """
        يفحص النص ويعيد:
        - True + قائمة بالأسباب إن كان طلب مساعدة
        - False + [] إن لم يكن
        """
        if not text:
            return False, []
        ...
        # 1. فحص السبام أولاً
        for spam in cls.SPAM_INDICATORS:
            if spam.lower() in text_lower:
                return False, []

        # 2. تجاهل الرسائل التي هي مجرد روابط
        if text_str.startswith('http') and len(text_str.split()) <= 3:
            return False, []

        reasons = []

        # 3. فحص علامات الاستفهام
        has_question = any(qm in text_str for qm in cls.QUESTION_MARKS)
        if has_question:
            reasons.append("سؤال")
        ...
        for pattern in cls.REQUEST_PATTERNS:
            ...
            if matches:
                request_found.extend(matches[:2])  # أول مبارين فقط
        if request_found:
            reasons.append("طلب: " + ", ".join(set(request_found[:3])))

        # 5. فحص السياق الدراسي
        context_found = []
        for ctx in cls.ACADEMIC_CONTEXT:
            if ctx.lower() in text_lower:
                context_found.append(ctx)
        if context_found:
            reasons.append("دراسي: " + ", ".join(context_found[:3]))

        # 6. منطق القرار:
        # - طلب + سياق دراسي = طلب مساعدة (تأكيد عالي)
        # - سؤال + سياق دراسي = طلب مساعدة (تأكيد عالي)
        # - طلب فقط + طول كافٍ = طلب مساعدة محتمل
        # - سؤال فقط + طول كافٍ = طلب مساعدة محتمل
        # - سياق دراسي فقط + طول كافٍ = محتمل

        has_request = bool(request_found)
        has_context = bool(context_found)

        # قرار نهائي
        is_help = False
        if has_request and has_context:
            is_help = True  # تأكيد عالي
        elif has_question and has_context:
            is_help = True  # تأكيد عالي
        elif has_request and len(text_str) >= 30:
            is_help = True  # محتمل
        elif has_question and len(text_str) >= 30:
            is_help = True  # محتمل
        elif has_context and len(text_str) >= 50 and (has_question or has_request):
            is_help = True  # تأكيد متوسط
