...
class MessageFormatter:
    ...
    @staticmethod
    def format_help():
        return ("🤖 أوامر بوت طلبات المساعدة v7\n\n"
                "📌 الأوامر العامة:\n"
                "• /help — هذه القائمة\n"
                "• /status — حالة البوت\n"
                "• /watchers — عرض المستخدمين المراقبين\n\n"
                "📌 المسح التاريخي (للمالك):\n"
                "• /scan_week — آخر 7 أيام\n"
                "• /scan_month — آخر 30 يوم\n"
                "• /scan_60 — آخر 60 يوم\n"
                "• /scan_full — كامل\n"
                "• /scan_stop — إيقاف\n\n"
                "📌 الإدارة (للمالك):\n"
                "• /add_watcher — إضافة مستخدم مراقب\n"
                "• /remove_watcher — إزالة مستخدم\n"
                "• /reset_scan — إعادة تعيين المسح\n")

    @staticmethod
    def format_status(total_requests, watchers_count, scan_running, scan_progress=""):
        return (f"📊 حالة البوت v7\n\n"
                f"📚 إجمالي طلبات المساعدة: {total_requests}\n"
...
class Monitor:
    ...
    def _register_handlers(self):
        ...
        logging.info("Bot handlers registered (channel + private chat for /start /login)")
    ...
    async def _handle_start(self, event, sender):
        ...
        welcome = (
            ...
            "👥 المستخدمون المراقبون حالياً: " + str(len(watchers)) + "\n\n"
            "🚀 للبدء، أرسل:\n"
            "/login - لتسجيل الدخول بحسابك الشخصي\n\n"
            "⚠️ ملاحظة: نحتاج رقم هاتفك + كود تيليجرام فقط.\n"
            "   لا تحتاج تثبيت أي تطبيق إضافي!"
        )
